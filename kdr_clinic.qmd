...
---
title: "Key Driver Summary (Procedural Areas)"
format: html
editor: visual
params:
  service : "OU"
...
} else if(params$service =='MD'){
  outvar<-"O4"
  outvar_all<-c("CP10","O3","O4") # Virtual Health
} else if (params$service =='OU'){
  outvar<-"E4"
  outvar_all<-c("E7", "E14","E4") # Outpatient Services
} else {
  outvar<-"O3"
  outvar_all<-c("O3") # Inpatient
}
...
tbwide<-data%>%
  ...
  select(-c(survey_id)) #temporary code removes problematic column
