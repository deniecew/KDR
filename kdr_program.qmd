...
#Percentile Rank
indiv_rank<-data %>%
  select(survey_id,program,type,varname,top_box) %>% #subset original data set
  group_by(program,varname) %>%
...
The [percentile rank]{.underline} is an internal rank that compares all MMG Programs within the Outpatient Oncology service line. The rank is calculated by counting the number of scores less than the program's top box score (x) and dividing by the number of distinct program scores (n). A percentile rank of 90 means the score is better than 90% of all program's scores within Moffitt.
