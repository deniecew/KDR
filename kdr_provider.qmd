...
if (params$service =='ON'){
  outvar<-"O7"
  outvar_all<-c("MED7","O2","O3","O7","I69") #Outpatient Oncology
} else {
  outvar<-"CP10"
...
r<-data%>%
  ...
  pivot_wider(names_from = varname, values_from = response) %>% #convert from long to wide format
...
The [percentile rank]{.underline} is an internal rank that compares all **`r params$type`s** within the Outpatient Virtual Health service line. It is calculated by counting the number of scores less than the provider's top box score (x) and dividing by the number of distinct provider's scores (n). A percentile rank of 90 means the score is better than 90% of all providers' scores within Moffitt.
